...
export default function PtPage() {
  ...
  const ptData = useMemo(() => {
    const hierarchical: Record<string, Record<string, typeof flatPtData>> = {};
    flatPtData.forEach(item => {
  ...
  const pagination = usePagination(flatPtData, 10);

  useEffect(() => {
    pagination.reset();
  }, [selectedUnit, selectedDepartment, selectedPeriod, selectedMonth, selectedYear]);

  const paginatedPtData = useMemo(() => {
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-8">
              {Object.entries(paginatedPtData).length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  No contributions found.
                </div>
              ) : (
                Object.entries(paginatedPtData).map(([unitName, departments]) => (
